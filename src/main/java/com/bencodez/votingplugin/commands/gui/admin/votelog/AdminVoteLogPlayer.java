...
/**
 * Admin GUI: VoteLog entries for a given player (paged).
 *
 * Uses VoteLogMysqlTable instance passed in from CommandLoader.
 *
 * Supports input as player name OR uuid string (you decide in CommandLoader
 * which constructor to call).
 */
public class AdminVoteLogPlayer extends GUIHandler {
	...
	public void onChest(Player player) {
		try {
			...
			List<VoteLogMysqlTable.VoteLogEntry> rows = table.getByPlayerName(playerName, days, needed);
